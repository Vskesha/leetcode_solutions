...
class Solution2:
    def braceExpansionII(self, expression: str) -> List[str]:
        ...
        def dfs():
            ...
            while i < len(expression) and (expression[i] == '{' or expression[i].isalpha()):
...
def test():
    sol = Solution()
    print('Test 1 ... ', end='')
    assert sol.braceExpansionII("{a,b}{c,{d,e}}") == ["ac", "ad", "ae", "bc", "bd", "be"]
    print('ok\nTest 2 ... ', end='')
    assert sol.braceExpansionII("{{a,z},a{b,c},{ab,z}}") == ["a", "ab", "ac", "z"]
    print('ok\nTest 3 ... ', end='')
    assert sol.braceExpansionII("n{g,{u,o}}{a,{x,ia,o},w}") == ['nga', 'ngia', 'ngo', 'ngw', 'ngx', 'noa', 'noia', 'noo', 'now', 'nox', 'nua', 'nuia', 'nuo', 'nuw', 'nux']
    print('ok')
