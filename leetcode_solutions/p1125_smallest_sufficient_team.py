...
class Solution:
    def smallestSufficientTeam(
        ...
        people_skills = [
            {skill_indices[skill] for skill in skills} for skills in people
        ]
        ...
        while bfs:
            ...
            rarest_skill = min(
                skills_remain, key=lambda x: len(skills_possessors[x])
            )
    ...
    def smallestSufficientTeam(
        ...
        while queue:
            ...
            for person_id in rarest_skill_possessors:
                remaining_skills = [
                    possessor
                    for possessor in top_skills
                    if person_id not in possessor
                ]
...
def test():
    ...
    assert 2 == len(
        sol.smallestSufficientTeam(
            ...
            people=[["mmcmnwacnhhdd"], [], [], ["vza", "mrxyc"]],
        )
    )
    print("ok")
